//Prohibido Modificar este C�digo 
...
void ListaCircularDoble::burbuja(NodoDoble* actual, NodoDoble* siguiente, bool intercambio)
{
	...
	{
		if (std::stoi(actual->getDato().getCedula()) > std::stoi(siguiente->getDato().getCedula()))
		{
			Persona temp = actual->getDato();
		...
		burbuja(actual, siguiente, intercambio);
	}



	if (intercambio) {
		burbuja(cabeza, cabeza->getSiguiente(), intercambio);
	}
	else {
		return;
	}
