...
void Archivos::guardarListaRegistro(ListaRegistro* lista) {
    ...
    if (archivo.is_open()) {
        for (int i = 0; i < lista->dimencion(); i++) {
            ...
            archivo << lista->getPosicion(i).getSalida().getSegundos() << ",";
        }
        archivo.close();
    ...
    if (archivo.is_open()) {
        ...
        while (std::getline(archivo, linea)) {
            ...
            if (coma_pos != std::string::npos) {
                ...
                eah = std::stoi(linea.substr(coma_pos10 + 1, coma_pos11 - coma_pos10 - 1));
                eami = std::stoi(linea.substr(coma_pos11 + 1, coma_pos12 - coma_pos11 - 1));
                eas = std::stoi(linea.substr(coma_pos12 + 1, coma_pos13 - coma_pos12 - 1));
                ...
                Fecha entrada(ed, em, ea,eah,eami,eas);
...
void Archivos::guardarListaEmpleados(ListaEmpleados* lista) {
    ...
    if (archivo.is_open()) {
        for (int i = 0; i < lista->dimencion(); i++) {
            ...
            archivo << lista->getPosicion(i).getSalario() << ",";
        }
        archivo.close();
